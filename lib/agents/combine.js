...
// 对齐 A2/A3 → 跨模板混搭护栏 → resolveTemplate（取真死模板 figure）→
// board / markdown / filmstripModel / guidance / editingFlow。
// figure 原语直接来自真库（lib/figure.js 契约：line.from/to、rect.at/size、
// circle.at/r、polyline.points=[[x,y]]），本模块不改写字段，原样透传给渲染器。

import { getById, nearestTemplate, resolveTemplate } from '../library.js';
...
// 跨模板混搭护栏：不同运镜（distinct camera）总数 ≤ 3。
// 当已用满 3 种且当前镜会引入第 4 种运镜时，用 nearestTemplate 从真库换一条
// 落在已用运镜集合里的模板（仍命中真库，仍由 getById/resolveTemplate 取真 figure）。
...
// filmstripModel：对齐架构设计 §4.2。cells[].figure 为真库原语；
// transitionToNext 末镜为 null；maxDuration 供节奏条长度归一化。
export function buildFilmstripModel(board) {
  const shots = board.shots;
  const maxDuration = shots.reduce((max, shot) => Math.max(max, shot.duration), 1);
  return {
    palette: { bg: '#000', fg: '#40FF5E' },
    shotCount: board.shotCount,
    totalDuration: board.totalDuration,
    maxDuration,
    cells: shots.map((shot, index) => ({
      index: shot.index,
      ...
      transition: shot.transition.id,
      transitionToNext: index < shots.length - 1 ? shot.transition.id : null,
      pause: shot.pause,
...
export function combineStoryboard(intent = {}, compositionResult = {}, rhythmResult = {}) {
  ...
  for (let i = 0; i < shotCount; i++) {
    ...
    // figure 必经真库：getById 命中后取其 figure（真原语）。
    const libTemplate = getById(template.id) || template;
    shots.push({
      index: i + 1,
      title: `镜头 ${i + 1}`,
