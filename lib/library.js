...
import { CAMERA_MOVES, COMPOSITIONS, SHOT_SIZES, pick } from './vocab.js';
...
function scoreTemplate(template, query) {
  ...
  if (query.shotSize && template.caption.shotSize === query.shotSize) score += 2;
  // A4 跨模板混搭护栏（运镜去重）用：允许 / 排斥某些运镜。
  if (Array.isArray(query.allowedCameras) && query.allowedCameras.includes(template.caption.camera)) score += 5;
  if (Array.isArray(query.disallowedCameras) && query.disallowedCameras.includes(template.caption.camera)) score -= 8;
  // A2 兜底跨模板混搭用（additive，缺省不生效）：对已用构图给负偏置，倾向不同构图。
  if (Array.isArray(query.disallowedCompositions) && query.disallowedCompositions.includes(template.caption.composition)) score -= 3;
  return score;
}
...
/**
 * 取模板并附带其 caption 词表项解析（shotSize/composition/camera → {id,...}）。
 * 缺失 / 非法 id 回退到库内第一条模板。供 A4 combine 直接读 label/desc/arrow。
 */
export function resolveTemplate(id, library = FROZEN_LIBRARY) {
  const template = getById(id, library) || templatesOf(library)[0] || null;
...
export function nearestTemplate(query = {}, library = FROZEN_LIBRARY) {
  // additive 选项 excludeIds：硬排除已选模板 id（用于兜底跨镜去重）；
  // 缺省 / 空数组时行为与历史完全一致。
  const excludeIds = Array.isArray(query.excludeIds) && query.excludeIds.length
    ? new Set(query.excludeIds)
    : null;
  // additive 选项 onlyCameras：硬约束候选只在给定运镜集合内（用于守 distinct(camera) ≤ 3）；
  // 缺省 / 空数组时不生效，行为与历史完全一致。
  const onlyCameras = Array.isArray(query.onlyCameras) && query.onlyCameras.length
    ? new Set(query.onlyCameras)
    : null;
  ...
  for (const template of templatesOf(library)) {
    if (excludeIds && excludeIds.has(template.id)) continue;
    if (onlyCameras && !onlyCameras.has(template.caption.camera)) continue;
