...
const STATIC_TEMPLATE_BY_ID = new Map(STORYBOARD_LIBRARY.templates.map((template) => [template.id, template]));

function templatesOf(library = STORYBOARD_LIBRARY) {
  return Array.isArray(library?.templates) ? library.templates : [];
}

function byIdFor(library = STORYBOARD_LIBRARY) {
  if (library === STORYBOARD_LIBRARY) return STATIC_TEMPLATE_BY_ID;
...
export function getById(id, library = STORYBOARD_LIBRARY) {
  if (typeof id !== 'string' || id.trim() === '') return null;
  return byIdFor(library).get(id) ?? null;
}

export function buildLibraryIndexForPrompt(library = STORYBOARD_LIBRARY) {
...
export function nearestTemplate(query = {}, library = STORYBOARD_LIBRARY) {
  let best = null;
  let bestScore = -1;
